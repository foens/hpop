...
namespace OpenPop.Async.Pop3
{
    ...
    /// </example>
    public class Pop3AsyncClient : Disposable
    {
        ...
        public async Task ConnectAsync(Stream stream)
        {
            ...
            string response = await StreamUtility.ReadLineAsAsciiAsync(Stream);
        ...
        public async Task ConnectAsync(string hostname, int port, bool useSsl)
        {
            ...
            await ConnectAsync(hostname, port, useSsl, defaultTimeOut, defaultTimeOut, null);
        }
        ...
        public async Task ConnectAsync(string hostname, int port, bool useSsl, int receiveTimeout, int sendTimeout, RemoteCertificateValidationCallback certificateValidator)
        {
            AssertDisposed();
        ...
        {
            AssertDisposed();
            await AuthenticateAsync(username, password, AuthenticationMethod.Auto);
        }
        ...
        /// <exception cref="ArgumentNullException">If <paramref name="username"/> or <paramref name="password"/> is <see langword="null"/></exception>
        /// <exception cref="LoginDelayException">If the server rejects the login because of too recent logins</exception>
        public async Task AuthenticateAsync(string username, string password, AuthenticationMethod authenticationMethod)
        {
            AssertDisposed();
            ...
            try
            {
                switch (authenticationMethod)
                {
                    case AuthenticationMethod.UsernameAndPassword:
                        await AuthenticateUsingUserAndPasswordAsync(username, password);
                        break;

                    case AuthenticationMethod.Apop:
                        await AuthenticateUsingApopAsync(username, password);
                        break;
                    ...
                    case AuthenticationMethod.CramMd5:
                        await AuthenticateUsingCramMd5Async(username, password);
                        break;
                }
            }
            catch (PopServerException e)
        ...
        private async Task AuthenticateUsingUserAndPasswordAsync(string username, string password)
        {
        ...
        /// <exception cref="PopServerException">If the server responded with -ERR</exception>
        private async Task AuthenticateUsingApopAsync(string username, string password)
        {
            if (!ApopSupported)
                throw new NotSupportedException("APOP is not supported on this server");

            await SendCommandAsync("APOP " + username + " " + Apop.ComputeDigest(password, ApopTimeStamp));
        ...
        /// <exception cref="PopServerLockedException">If the server said the the mailbox was locked</exception>
        /// <exception cref="LoginDelayException">If the server rejects the login because of too recent logins</exception>
        private async Task AuthenticateUsingCramMd5Async(string username, string password)
        {
            ...
            try
            {
                await SendCommandAsync("AUTH CRAM-MD5");
            }
            catch (PopServerException e)
            {
        ...
        public async Task<int> GetMessageCountAsync()
        {
            ...
            return await SendCommandIntResponseAsync("STAT", 1);
        }
        ...
        public async Task DeleteMessageAsync(int messageNumber)
        {
            AssertDisposed();
            ...
            await SendCommandAsync("DELE " + messageNumber);
        }        
        ...
        public async Task DeleteAllMessagesAsync()
        {
            ...
            int messageCount = await GetMessageCountAsync();
        ...
        public async Task NoOperationAsync()
        {
            AssertDisposed();
        ...
        public async Task<string> GetMessageUidAsync(int messageNumber)
        {
            AssertDisposed();
            ...
            await SendCommandAsync("UIDL " + messageNumber);
        ...
        public async Task<List<string>> GetMessageUidsAsync()
        {
            AssertDisposed();
            ...
            while (!IsLastLineInMultiLineResponse(response = await StreamUtility.ReadLineAsAsciiAsync(Stream)))
            {
        ...
        public async Task<int> GetMessageSizeAsync(int messageNumber)
        {
            AssertDisposed();
            ...
            // C: LIST 2
            // S: +OK 2 200
            return await SendCommandIntResponseAsync("LIST " + messageNumber, 2);
        }        
        ...
        /// <exception cref="PopServerException">If the server did not accept the LIST command</exception>
        public async Task<List<int>> GetMessageSizesAsync()
        {
            AssertDisposed();
            ...
            while (!".".Equals(response = await StreamUtility.ReadLineAsAsciiAsync(Stream)))
            {
       ...
       /// <summary>
        ...
        public async Task<Message> GetMessageAsync(int messageNumber)
        {
            AssertDisposed();
            ...
            byte[] messageContent = await GetMessageAsBytesAsync(messageNumber);
        ...
        /// <exception cref="PopServerException">If the server did not accept the command sent to fetch the message</exception>
        public async Task<byte[]> GetMessageAsBytesAsync(int messageNumber)
        {
            AssertDisposed();
            ...
            return await GetMessageAsBytesAsync(messageNumber, false);
        }
        ...
        /// <exception cref="PopServerException">If the server did not accept the command sent to fetch the message</exception>
        public async Task<MessageHeader> GetMessageHeadersAsync(int messageNumber)
        {
            AssertDisposed();
            ...
            byte[] messageContent = await GetMessageAsBytesAsync(messageNumber, true);
        ...
        public async Task<Dictionary<string, List<string>>> CapabilitiesAsync()
        {
            AssertDisposed();
            ...
            while (!IsLastLineInMultiLineResponse(lineRead = await StreamUtility.ReadLineAsAsciiAsync(Stream)))
            {
        ...
        private void SendCommand(string command)
        {
            ...
            Stream.WriteAsync(commandBytes, 0, commandBytes.Length);
        ...
        /// <summary>
        /// Sends a command to the POP server asynchronously.<br/>
        /// If this fails, an exception is thrown.
        /// </summary>
        /// <param name="command">The command to send to server</param>
        /// <exception cref="PopServerException">If the server did not send an OK message to the command</exception>
        private async Task SendCommandAsync(string command)
        {
            ...
            // Read the response from the server. The response should be in ASCII
            LastServerResponse = await StreamUtility.ReadLineAsAsciiAsync(Stream);
        ...
        private async Task<int> SendCommandIntResponseAsync(string command, int location)
        {
        ...
        /// <exception cref="PopServerException">If the server did not accept the command sent to fetch the message</exception>
        private async Task<byte[]> GetMessageAsBytesAsync(int messageNumber, bool askOnlyForHeaders)
        {
            AssertDisposed();
            ...
            if (askOnlyForHeaders)
            {
                ...
                await SendCommandAsync("TOP " + messageNumber + " 0");
            }
            else
            {
                // Ask for the full message
                await SendCommandAsync("RETR " + messageNumber);
            }
            ...
            using (MemoryStream byteArrayBuilder = new MemoryStream())
            {
                ...
                // Keep reading until we are at the end of the multi line response
                while (!IsLastLineInMultiLineResponse(lineRead = await StreamUtility.ReadLineAsBytesAsync(Stream)))
                {
